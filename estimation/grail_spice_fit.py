...
def run_spice_fit(inputs):
    ...
    start_time = (
        time_conversion.datetime_to_tudat(inputs[1]).to_epoch_time_object() + 600.0
    )
    ...
    with util.redirect_std(
        ...
        integrator_settings = propagation_setup.integrator.runge_kutta_fixed_step_size(
            numerical_simulation.Time(0, integration_step),
        ...
        estimation_output = estimation.create_best_fit_to_ephemeris(
            bodies,
            acceleration_models,
            bodies_to_propagate,
            central_bodies,
            ...
            numerical_simulation.Time(0, 60.0),
            extra_parameters,
