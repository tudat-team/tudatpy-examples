...
def compute_station_shift(residuals, p_old, sat_positions, sat_velocities, transmitting_station, delta=1e3):
    """
...
ifms_file = 'mex_phobos_flyby/ifms/filtered/M14ODFXL02_DPX_133631130_00.TAB'
fdets_residuals_file = 'mex_phobos_flyby/output/fdets_residuals/ONSALA60_residuals.csv'
fdets_file = 'mex_phobos_flyby/fdets/complete/Fdets.mex2013.12.28.On.complete.r2i.txt'
...
# Read the file
with open(fdets_residuals_file, mode="r") as file:
    ...
    for row in reader:
        ...
        fdets_times.append(float(row[0]))
        fdets_utc_times.append(datetime.strptime(row[1], "%Y-%m-%d %H:%M:%S"))
        fdets_residuals.append(float(row[2]))
...
mex_antenna_velocities = []
mex_antenna_positions = []
mex_ephemeris = bodies.get('MEX').ephemeris
antenna_state = antenna_ephemeris.cartesian_state(fdets_times[0])
for epoch in fdets_times:
    mex_state = mex_ephemeris.cartesian_state(epoch)
    mex_antenna_velocities.append(mex_state[3:])
    mex_antenna_positions.append(np.array(antenna_state[:3]) + np.array(mex_state[:3]))

stations_list = environment_setup.ground_station.radio_telescope_stations()
receiving_station_name = get_fdets_receiving_station_name(fdets_file)
for i in range(len(stations_list)):
    station_name = stations_list[i].station_name
    if str(receiving_station_name) == station_name:
        print(f'i: {i}')
        old_station_position = stations_list[i].station_position
        print('old_station_position', old_station_position)
        break

new_station_position, delta_shift = compute_station_shift(fdets_residuals, old_station_position, mex_antenna_positions, mex_antenna_velocities, transmitting_station_name)
stations_list[i].station_position = new_station_position
body_settings.get('Earth').ground_station_settings = stations_list
print('new_station_position', stations_list[i].station_position)

filtered_collections_list = []
transmitting_stations_list = []
single_ifms_collections_list = []

fdets_files = [fdets_file]
ifms_files = []
...
for fdets_file in fdets_files:
    ...
    for ifms_file in ifms_files:
        ...
        for filtered_collection, transmitting_station_name in zip(filtered_collections_list,transmitting_stations_list):
            ...
            filtered_times = filtered_collection.get_concatenated_observation_times()
            filtered_times = [time.to_float() for time in filtered_times]

            filtered_mjd_times = [time_conversion.seconds_since_epoch_to_julian_day(t) for t in filtered_times]
            filtered_utc_times = np.array([Time(mjd_time, format='jd', scale='utc').datetime for mjd_time in filtered_mjd_times])
            ...
            if site_name not in fdets_station_residuals.keys():
                fdets_station_residuals[site_name] = [(filtered_times, filtered_utc_times, concatenated_residuals, mean_residuals, rms_residuals)]
            else:
                fdets_station_residuals[site_name].append((filtered_times, filtered_utc_times, concatenated_residuals, mean_residuals, rms_residuals))
...
for site_name, data_list in fdets_station_residuals.items():
    ...
    for filtered_times, filtered_utc_times, residuals, mean_residuals, rms_residuals in data_list:
        print(len(filtered_utc_times), len(residuals))
        # Plot all stations' residuals on the same figure
        plt.scatter(
            filtered_utc_times, residuals,
...
with open(fdets_residuals_file, mode="r") as file:
    ...
    plt.scatter(fdets_utc_times,fdets_residuals, marker = '+', s=5, label ='old residuals')
