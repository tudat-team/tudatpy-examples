...
def get_grail_files(local_path, start_date, end_date):
    ...
    if os.path.exists(local_path + manoeuvres_file) == False:
        ...
        # If not, download it
        urlretrieve(
            url_manoeuvres_files + manoeuvres_file, local_path + manoeuvres_file
        )
    ...
    antenna_files = download_url_files_time(
        local_path=local_path,
        filename_format="*/vgs1b_*_a_04.asc",
        start_date=start_date,
        end_date=end_date,
        url=url_antenna_files,
        time_format="%Y_%m_%d",
    ...
    if os.path.exists(local_path + grail_frames_def_file) == False:
        ...
        # If not, download it
        urlretrieve(
            url_frames_def_file + grail_frames_def_file,
            local_path + grail_frames_def_file,
        )
    ...
    if os.path.exists(local_path + moon_orientation_file) == False:
        ...
        # If not, download it
        urlretrieve(
            url_moon_orientation_file + moon_orientation_file,
            local_path + moon_orientation_file,
        )
    ...
    if os.path.exists(local_path + lunar_frame_file) == False:
        ...
        # If not, download it
        urlretrieve(
            url_lunar_frame_file + lunar_frame_file, local_path + lunar_frame_file
        )
...
def get_grail_panel_geometry():
    ...
    panel_data = pd.read_csv(
        this_file_path + "/grail_inputs/grail_macromodel.txt",
        delimiter=", ",
        engine="python",
    )
    material_data = pd.read_csv(
        this_file_path + "/grail_inputs/grail_materials.txt",
        delimiter=", ",
        engine="python",
    )
    ...
    for i, row in panel_data.iterrows():
        ...
        panel_geometry_settings = (
            environment_setup.vehicle_systems.frame_fixed_panel_geometry(
                np.array(
                    [row["x"], row["y"], row["z"]]
                ),  # panel position in body reference frame
                row["area"],
            )
        )  # panel area

        # Retrieve panel material data
        panel_material_data = material_data[
            material_data["material"] == row["material"]
        ]
        ...
        specular_diffuse_body_panel_reflection_settings = (
            environment_setup.radiation_pressure.specular_diffuse_body_panel_reflection(
                ...
                diffuse_reflectivity=float(panel_material_data["Cd"].iloc[0]),
        ...
        complete_panel_settings = environment_setup.vehicle_systems.body_panel_settings(
            panel_geometry_settings, specular_diffuse_body_panel_reflection_settings
        )
    ...
    full_panelled_body_settings = (
        environment_setup.vehicle_systems.full_panelled_body_settings(
            all_panel_settings
        )
    )
