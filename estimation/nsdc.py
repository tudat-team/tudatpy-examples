...
def nsdc_moon_name(moon_id):
    ...
    if moon_id in moon_names:
        return moon_names[moon_id]
    else:
        raise ValueError(f"Unknown moon ID: {moon_id}")

def nsdc_moon_ID (moon, central_body):     #Numbers, e.g. 5, 5 for Amalthea
    '''
    ...
    .. code-block:: python
        ...
        body = nsdc_moon_ID(moon, central_body)
...
# DOMINIC TODO: requires observatory file; to be added to resources
...
def return_standardised_time(values, time_entry, time_scale, amount_time_entries, time_delta,
                             time_type):  # Meta-function for standardised time
    '''
...
def return_standardised_rotation_ECLIPJ2000(RA_rad, DEC_rad, epoch,
                                            orientation):  # Convert from any orientation to ECLIPJ2000
    '''
...
def read_nsdc_file_settings(filename):
    '''
    ...
    .. code-block:: python
        ...
        meta_data = read_nsdc_file_settings(filename)
    ...
    process_relative_nsdc_observation: bool
    ...
    process_relative_nsdc_observation = bool(first_words[13])              #Switch for processing relative observations or not
    orientation = first_words[14]
    central_body =  first_words[15]

    return (data_type,moon_entry,time_entry,amount_time_entries,time_type,time_scale,time_delta,RA_entry,DEC_entry,number_observatories,observatory,observatory_entrace,relative_body_entry,process_relative_nsdc_observation,orientation,central_body)

def read_nsdc_file_line(observation,number_observatories,observatory,observatory_entrace,moon_entry,central_body):    #retrieving data from line
    '''
    ...
    if moon_entry.isalpha():
        ...
        body = nsdc_moon_ID(values[int(moon_entry)],central_body)  

    return body, observatory, values

def create_nsdc_observation_set(file,bodies,targets_to_use):
    '''
    ...
    targets_to_use: list[str]
        bodies for which to keep the observations (all if empty)
    ...
    .. code-block:: python
        ...
        for file in raw_observation_files:
            observation_set, observation_settings = create_nsdc_observation_set(file,bodies)
    ...
    # DOMINIC TODO: allow existing ground stations to be used
    if( ~( Observatory in bodies.get_body("Earth").ground_station_list) ):
    ...
    # Define link ends
    link_ends = dict()
    link_ends[observation.transmitter] = observation.body_origin_link_end_id(nsdc_moon_name(Moon))
...
def create_nsdc_observation_collection(files,bodies,targets_to_use=[]):
    observation_set_list = []
    ...
    for file in files:
        ...
        if (len(targets_to_use) == 0 or Moon in targets_to_use):

            observation_set, observation_settings = create_nsdc_observation_set(file, bodies,targets_to_use)
            observation_set_list.append(observation_set)
...
def analyse_nsdc_data(times,observations, moons, observatories, bodies, plotting,standard_orientation):
    ...
    .. code-block:: python
        ...
        means, stddevs, diflist = analyse_nsdc_data(times, RADEC, moons, observatories, bodies, True, 'ECLIPJ2000')
...
def remove_nsdc_outliers(times, observations, moons, observatories, bodies,standard_orientation, outlier_limit = 3, plotting= True):
    ...
    .. code-block:: python
        bodies = load_standard_jupiter_EJ2000()
        filtered_times, filtered_observations, filtered_moons, filtered_observatories, filtered_diflist, rms, means, stddevs = remove_nsdc_outliers(times, observations, moons, observatories, bodies, 'ECLPIJ2000', 3, True)
...
def add_observatory(bodies,observatory):       #Generate ephemerides for the observatory
    ...
    -----------
    .. code-block:: python
        bodies = generate_standard_nsdc_environment('Jupiter','ECLIPJ2000)
...
def generate_standard_nsdc_environment(central_body, standard_orientation, extra_bodies=[], global_frame_origin = 'Earth' ):
    '''
    ...
    if central_body == 'Jupiter':
        standard_moons = ['501', '502', '503', '504', '505','506','514', '515', '516']
...
def process_relative_nsdc_observation(values,RA_entry,DEC_entry,body,relative_body,orientation,observatory,epoch,data_type,bodies,standard_orientation):                   #Optional function for processing the relative data
    ...
    -----------
    .. code-block:: python
        RA_rad, DEC_rad = process_relative_nsdc_observation(values,RA_entry,DEC_entry,body,relative_body,orientation,observatory,epoch,data_type)
...
def return_standardised_angles(values,RA_entry,DEC_entry,body,relative_body_entry,orientation,observatory,epoch,data_type,central_body,bodies,standard_orientation): #Meta-function for standardised output
    ...
    if data_type=='REL' or data_type=='DIF' or data_type == 'REL_wo_cos':
        ...
        if relative_body_entry.isalpha():
            ...
            relative_body = nsdc_moon_ID(values[int(relative_body_entry)],central_body)
        ...
        RA_rad, DEC_rad = process_relative_nsdc_observation(values,RA_entry,DEC_entry,body,relative_body,orientation,observatory,epoch,data_type,bodies,standard_orientation)
...
def save_nsdc_data_to_csv(times, RADEC, moons, observatories, studyname, diflist, output_folder):
    '''
    ...
    .. code-block:: python
        ...
        save_nsdc_data_to_csv(times, RADEC, moons, observatories, studyname)
    ...
    for moon in unique_moons:
        ...
        for observatory in unique_observatories:
            ...
            with open(output_folder + str(int(moon)) + '_' + str(observatory) + '_' + studyname + '.csv', 'w',
                      newline='') as csvfile:
                writer = csv.writer(csvfile)

                # Write header row
                writer.writerow(
                    ['Seconds since J2000', 'RA ECLIPJ2000 [rad]', 'DEC ECLIPJ2000 [rad]', 'O-C RA ECLIPJ2000 [rad]',
                     'O-C DEC ECLIPJ2000 [rad]'])
...
def process_nsdc_file(filename,analyse, standard_orientation):
    ...
    '''  
    #Meta_data
    data_type,moon_entry,time_entry,amount_time_entries,time_type,time_scale,time_delta,RA_entry,DEC_entry,number_observatories,observatory,observatory_entrace,relative_body_entry,process_relative_nsdc_observation,orientation,central_body = read_nsdc_file_settings(filename)
    with open(filename, 'r') as datafile:
        studyname = filename.split("/")[-1][:-4]
        # Create lists to store the data
        data, times, RADEC, moons,observatories = [],[],[],[],[]
...
folder_path = 'NsdcObservationsTest'
raw_observation_files = [os.path.join(folder_path, f) for f in os.listdir(folder_path) if f.endswith('.txt')]

# #Start loop over every txt in folder
for raw_file in raw_observation_files:
    process_nsdc_file(raw_file,True, 'ECLIPJ2000')

folder_path = 'ObservationsProcessedTest'
processed_observation_files = [os.path.join(folder_path, f) for f in os.listdir(folder_path) if f.endswith('.csv')]

bodies = generate_standard_nsdc_environment( 'Jupiter', 'ECLIPJ2000', global_frame_origin='SSB')
nsdc_observation_collection = create_nsdc_observation_collection( processed_observation_files, bodies, ['506'] )

print(nsdc_observation_collection.link_definitions_per_observable)
angular_position_link_ends = nsdc_observation_collection.link_definitions_per_observable[
    estimation_setup.observation.angular_position_type ]

observation_model_settings = list()
for current_link_definition in angular_position_link_ends:
    observation_model_settings.append(estimation_setup.observation.angular_position(
        current_link_definition ))
...
estimation.compute_residuals_and_dependent_variables(nsdc_observation_collection, observation_simulators, bodies)

rms_residuals = nsdc_observation_collection.get_rms_residuals()
mean_residuals = nsdc_observation_collection.get_mean_residuals()

# Filter residual outliers
nsdc_observation_collection.filter_observations(
    estimation.observation_filter(estimation.residual_filtering, 5.0E-5 ))
...
concateated_residuals = nsdc_observation_collection.get_concatenated_residuals()
concatenated_times = nsdc_observation_collection.concatenated_float_times
concatenated_link_definition_ids = nsdc_observation_collection.concatenated_link_definition_ids

plt.scatter(concatenated_times[::2], concateated_residuals[::2], c=concatenated_link_definition_ids[::2] )
plt.xlabel('Observation epoch [Julian years since J2000]')
plt.ylabel('spice-observed RA [rad]')
plt.grid()
plt.show()

plt.scatter(concatenated_times[1::2], concateated_residuals[1::2], c=concatenated_link_definition_ids[1::2] )
plt.xlabel('Observation epoch [Julian years since J2000]')
plt.ylabel('spice-observed DEC [rad]')
plt.grid()
plt.show()

plt.hist(concateated_residuals[::2], density=True, bins=30)
plt.show()

plt.hist(concateated_residuals[1::2], density=True, bins=30)
plt.show()
